...
# Docker Image화
[Docker](https://www.docker.com/) 는 쉽게 설명하면 특정 앱(지금 만드는 Go server같은 앱)을 가상의 환경에 구동할 수 있도록 해주는 앱이다. 

Docker에 대해 설명하는 포스트는 추후에 작성하도록 하겠다.

Docker Image화는 그 가상의 구동 환경을 저장시키는 것이다. 이론 상으로 다른 컴퓨터에서 Docker Image 만 Container로 띄우면 Image속에 저장된 Go server는 정상 작동된다. 
...
다음 포스트에서는 어떤 방식으로 서버를 배포할 것인지를 정한 뒤 배포까지 자동화하는 것을 알아보자.
